"""Reference-only: a rule-based agent for a Mega Lucario ex archetype, pasted into
chat by the user on 2026-08-16 and claimed to be a public Kaggle notebook scoring
664 on the leaderboard. That score is unverified by us -- treat it as the user's
claim, not a confirmed fact. Kept here for the diff against src/baseline.py and as
a record of what was actually reviewed; NOT wired into main.py, NOT our own work
product. See docs/writeup/mega_lucario_diff.md for the actual comparison.

Do not copy this deck (card IDs 673-678, 1102 etc. are a different archetype) or
submit this file as-is -- only specific, adapted ideas get ported into
src/baseline.py, per instruction.
"""
