...
def baseline_score(obs_dict: dict, option_idx: int) -> float:
    ...
    if context == int(SelectContext.SETUP_ACTIVE_POKEMON):
        # Ablation candidate A (docs/writeup/ablation_log.md): previously
        # unhandled, fell through to the 0.0 default below, so the
        # first-listed Basic always won by stable sort with no strategic
        # input. Prefer whichever Basic is closest to affording its own
        # attack -- cheap energy cost first since it can threaten sooner.
        area, idx = opt.get("area"), opt.get("index")
        hand = _player(obs_dict, your_index)["hand"] or []
        card_id = hand[idx]["id"] if area == 2 and idx is not None and idx < len(hand) else None
        card = _CARD_BY_ID.get(card_id)
        if card is None or not card.attacks:
            return 0.0
        min_cost = min((len(_ATTACK_BY_ID[a].energies) for a in card.attacks if a in _ATTACK_BY_ID), default=5)
        return 1.0 - min(min_cost, 5) / 5.0

    if context == int(SelectContext.TO_HAND):
        # Ablation candidate A. Previously unhandled (0.0 default). Relevant
        # to us: Ultra Ball, Poke Pad, Fighting Gong, and Boxed Order all
        # trigger a TO_HAND search choice, always searching the deck (area ==
        # AreaType.DECK) -- that's the only source handled here; anything
        # else is left at the 0.0 default rather than guessed at.
        area, idx = opt.get("area"), opt.get("index")
        card_id = None
        if area == 1 and select.get("deck") and idx is not None and idx < len(select["deck"]):  # DECK
            c = select["deck"][idx]
            card_id = c["id"] if c is not None else None
        card = _CARD_BY_ID.get(card_id)
        if card is None:
            return 0.0
        me = _player(obs_dict, your_index)
        have_basic_in_play = any(
            (in_play_card := _CARD_BY_ID.get(p["id"])) is not None and in_play_card.basic
            for p in (me["active"] + me["bench"])
        )
        if card.basic and not have_basic_in_play:
            return 1.0  # need a Basic on board before anything else matters
        if card.cardType == 5:  # BASIC_ENERGY
            return 0.6
        if card.cardType == 3:  # SUPPORTER
            return 0.5
        return 0.3

    if context == int(SelectContext.ATTACH_FROM):
        # Ablation candidate A. Previously unhandled (0.0 default). Not
        # currently triggered by any card in our deck (a plain hand-to-field
        # attach is a single MAIN-select ATTACH option that already bundles
        # its own target; ATTACH_FROM is the effect-driven path, e.g. "attach
        # an Energy from your discard to 1 of your Pokemon"), so this is a
        # completeness fix for contexts our own deck doesn't exercise yet,
        # not a strategy change -- flat score, same as MAIN-select ATTACH's
        # default, deliberately with no plan/role-awareness (that's a
        # separate candidate, not bundled here).
        area, idx = opt.get("area"), opt.get("index")
        pool: list = []
        if area == 4:
            a = _active(obs_dict, your_index)
            pool = [a] if a is not None else []
        elif area == 5:
            pool = _player(obs_dict, your_index)["bench"]
        if idx is not None and idx < len(pool) and pool[idx]:
            return 0.4
        return 0.0
