    ...
    if context == int(SelectContext.MAIN):
        ...
        if opt_type == 7:  # PLAY (from hand)
            ...
            if card.cardType == 3:  # SUPPORTER
                # Candidate E: resolve draw/search supporters before the
                # attach/attack decision, unconditionally, so the next MAIN
                # decision (freshly re-scored by the engine) sees the
                # post-draw hand. Outranks every other MAIN score, including
                # ATTACK's dmg/100 + prize_value (bounded well under 9.0 for
                # this game). Hard-override lethal/trade-up checks still run
                # before baseline_score at all, so this can't cause a missed
                # game-winning attack.
                return 9.0
