...
from src.switch_predict import predict_post_switch
from src.value_features import position_features
from src.value_net import ValueNet

# Step 4 of the value-net proposal (docs/writeup/ablation_log.md): blend
# weight for SWITCH-context (retreat-target) scoring only --
# final_score = (1-w)*M1_heuristic + w*value_net. w=0.0 (default) must
# reproduce the pure-heuristic control exactly; a config value, not a
# hardcoded constant, so eval scripts can sweep it.
VALUE_NET_WEIGHT = 0.0

_VALUE_NET_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))),
    "ckpt", "value_net.npz"
)  # this archived copy lives 4 levels below repo root (docs/writeup/rejected/), not 2 like src/baseline.py
_VALUE_NET = None
_VALUE_NET_LOAD_ATTEMPTED = False


def _get_value_net() -> ValueNet | None:
    """Lazily loads the trained value net; returns None (graceful
    heuristic-only fallback) if the checkpoint is missing or fails to load
    -- a missing checkpoint must never crash choose()."""
    global _VALUE_NET, _VALUE_NET_LOAD_ATTEMPTED
    if not _VALUE_NET_LOAD_ATTEMPTED:
        _VALUE_NET_LOAD_ATTEMPTED = True
        try:
            _VALUE_NET = ValueNet.load(_VALUE_NET_PATH)
        except Exception:
            _VALUE_NET = None
    return _VALUE_NET
...
def baseline_score(obs_dict: dict, option_idx: int) -> float:
    ...
    if context in (int(SelectContext.SWITCH), int(SelectContext.TO_ACTIVE)):
        ...
        if area == 5 and idx is not None:  # BENCH
            bench = _player(obs_dict, your_index)["bench"]
            if idx < len(bench):
                heuristic = bench[idx]["hp"] / max(bench[idx]["maxHp"], 1)
                # Step 4 blend: SWITCH only (predict_post_switch is only
                # validated for SWITCH, not TO_ACTIVE -- see Step 2).
                # w=0.0 short-circuits before touching the value net at
                # all, so the default path is byte-identical to control
                # and never depends on the checkpoint existing.
                if context == int(SelectContext.SWITCH) and VALUE_NET_WEIGHT > 0.0:
                    net = _get_value_net()
                    if net is not None:
                        predicted_obs = predict_post_switch(obs_dict, your_index, idx)
                        value_score = float(net.predict(position_features(predicted_obs, your_index))[0])
                        return (1 - VALUE_NET_WEIGHT) * heuristic + VALUE_NET_WEIGHT * value_score
                return heuristic
        return 0.0
