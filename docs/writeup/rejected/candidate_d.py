...
def _retreat_justified(obs_dict: dict, your_index: int) -> bool:
    """Candidate D gate: retreat only if the active can't attack profitably
    right now, some benched Pokemon could attack immediately if promoted, and
    paying the retreat cost won't fully drain the retreating Pokemon's own
    energy investment."""
    my_active = _active(obs_dict, your_index)
    if my_active is None:
        return False
    card = _CARD_BY_ID.get(my_active["id"])
    if card is None:
        return False
    opp_active = _active(obs_dict, 1 - your_index)
    opp_card_id = opp_active["id"] if opp_active else None
    ...
    for attack_id in card.attacks:
        ...
        if atk is None or not _can_pay(attached, atk.energies):
            continue
        dmg = _guaranteed_damage(my_active["id"], attack_id, opp_card_id) if opp_card_id else atk.damage
        if dmg > 0:
            return False  # can already attack profitably -- no reason to retreat

    bench = _player(obs_dict, your_index)["bench"] or []
    bench_ready = any(
        _can_pay(mon.get("energies") or [], _ATTACK_BY_ID[a].energies)
        for mon in bench
        if (bcard := _CARD_BY_ID.get(mon["id"])) is not None
        for a in bcard.attacks if a in _ATTACK_BY_ID
    )
    if not bench_ready:
        return False

    remaining = len(attached) - card.retreatCost
    if attached and remaining <= 0:
        return False  # would strand this Pokemon's energy investment
...
def baseline_score(obs_dict: dict, option_idx: int) -> float:
    ...
    if context == int(SelectContext.MAIN):
        ...
        if opt_type == 12:  # RETREAT
            return 0.5 if _retreat_justified(obs_dict, your_index) else -0.3
