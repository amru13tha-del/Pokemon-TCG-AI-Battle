...
def _energy_readiness(target: dict | None, provided_energy_type: int | None) -> tuple[bool, bool]:
    """(can_attack_before, can_attack_after) attaching one energy of
    provided_energy_type to target -- True if any of target's attacks would
    be payable, checked against target's currently attached energies."""
    if target is None or provided_energy_type is None:
        return False, False
    card = _CARD_BY_ID.get(target["id"])
    if card is None or not card.attacks:
        return False, False
    attached = target.get("energies") or []
    attacks = [_ATTACK_BY_ID[a] for a in card.attacks if a in _ATTACK_BY_ID]
    before = any(_can_pay(attached, a.energies) for a in attacks)
    after = any(_can_pay(attached + [provided_energy_type], a.energies) for a in attacks)
    return before, after
...
def baseline_score(obs_dict: dict, option_idx: int) -> float:
    ...
    if context == int(SelectContext.MAIN):
        ...
        if opt_type == 8:  # ATTACH (energy/tool)
            ...
            src_idx = opt.get("index")
            src_card = _CARD_BY_ID.get(hand[src_idx]["id"]) if src_idx is not None and src_idx < len(hand) else None
            if src_card is not None and src_card.cardType in (5, 6):  # BASIC_ENERGY / SPECIAL_ENERGY
                in_area, in_idx = opt.get("inPlayArea"), opt.get("inPlayIndex")
                if in_area == 4:  # ACTIVE
                    target = _active(obs_dict, your_index)
                elif in_area == 5:  # BENCH
                    bench = _player(obs_dict, your_index)["bench"]
                    target = bench[in_idx] if in_idx is not None and in_idx < len(bench) else None
                else:
                    target = None
                before, after = _energy_readiness(target, src_card.energyType)
                if after and not before:
                    return 0.5  # this attachment unlocks an attacker
                if not after:
                    return 0.3  # target still won't be able to attack -- deprioritize
            return 0.4
