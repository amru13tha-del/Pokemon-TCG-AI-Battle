"""Snapshot of src/baseline.py as it was immediately before the Mega Lucario
port (2026-08-16), reconstructed from the full-file Read at the start of that
work. Exists only so the 300-game evaluation compares the new baseline against
the actual prior version, not against itself. Not used anywhere else.
"""
...
def baseline_score(obs_dict: dict, option_idx: int) -> float:
    ...
    if context in (int(SelectContext.EFFECT_TARGET), int(SelectContext.DAMAGE)):
        area, idx = opt.get("area"), opt.get("index")
        if area in (4, 5) and idx is not None:
            opp_index = 1 - your_index
    ...
    if context == int(SelectContext.DISCARD):
        area, idx = opt.get("area"), opt.get("index")
        if area == 2 and idx is not None:
            hand = _player(obs_dict, your_index)["hand"] or []
            if idx < len(hand):
                card = _CARD_BY_ID.get(hand[idx]["id"])
                if card is not None:
                    return 0.5 if card.cardType in (5, 6) else -0.3
        return 0.0
